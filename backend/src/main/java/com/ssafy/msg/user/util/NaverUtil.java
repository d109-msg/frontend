...
@Component
@Slf4j
public class NaverUtil implements Oauth2Util {
	
	@Value("${oauth2.client.provider.naver.user-info-uri}")
	private String userInfoUri;
	
	@Value("${oauth2.client.provider.naver.token-uri}")
	...
	public Oauth2Dto getUserInfo(String naverAccessToken) throws Exception {
		try {
			RestTemplate restTemplate = new RestTemplate();
			HttpHeaders httpHeaders = new HttpHeaders();
			httpHeaders.add("Authorization", "Bearer " + naverAccessToken);
			httpHeaders.add("Content-type", "application/x-www-form-urlencoded;charset=utf-8");
			HttpEntity<MultiValueMap<String, String>> naverUserInfoRequest = new HttpEntity<>(httpHeaders);
			ResponseEntity<String> response = restTemplate.exchange(userInfoUri, HttpMethod.POST, naverUserInfoRequest,
					String.class);
			JsonElement jsonElement = JsonParser.parseString(response.getBody());
			JsonObject jsonObject = jsonElement.getAsJsonObject().get("response").getAsJsonObject();
			String nickname = jsonObject.get("nickname").getAsString();
			String emailId = jsonObject.get("email").getAsString();
			return Oauth2Dto.builder().nickname(nickname).emailId(emailId).provider("naver").build();
		} catch (Exception e) {
